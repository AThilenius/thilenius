...
class Mapper {
 public:
  ...
  ~Mapper() {
    for (typename std::vector<Member*>::iterator i = m_fields.begin();
         i != m_fields.end(); ++i) {
      delete *i;
    }
  }
  ...
  Mapper(Mapper const& mapper) {
    for (typename std::vector<Member*>::const_iterator i =
             mapper.m_fields.begin();
         i != mapper.m_fields.end(); ++i) {
  ...
  Mapper& add_field(std::string const& name, U T::*field) {
    m_fields.push_back(
        direct_member<U>(name, member_direct(field), BasicCoder<U>()));
    return *this;
  }

  template <typename U, typename Alloc>
  Mapper& add_field(std::string const& name, std::vector<U, Alloc> T::*field,
                    Mapper<U> const& mapper) {
    m_fields.push_back(
        direct_member<U>(name, member_direct(field),
  ...
  Mapper& add_field(std::string const& name, U const& (T::*getter)() const,
    ...
    m_fields.push_back(indirect_member<U>(
        name, member_fxns_indirect<U const&>(getter, setter), BasicCoder<U>()));
    return *this;
  }

  template <typename U>
  Mapper& add_field(std::string const& name, U (T::*getter)() const,
                    void (T::*setter)(U)) {
    m_fields.push_back(indirect_member<U>(
        name, member_fxns_indirect<U>(getter, setter), BasicCoder<U>()));
    return *this;
  }

  template <typename U>
  std::string const& lookup_field(U T::*member) const {
    for (typename std::vector<Member*>::const_iterator i = m_fields.begin();
         i != m_fields.end(); ++i) {
  ...
  std::string to_json(T const& t) const { return to_bson(t).jsonString(); }
 ...
 private:
  ...
  template <typename U, typename ACCESSOR, typename CODER>
  static DirectMember<U, ACCESSOR, CODER>* direct_member(
      std::string const& name, ACCESSOR accessor, CODER coder) {
   ...
   public:
    ...
    void from_bson(mongo::BSONObj const& bson, T& t) const {
      if (not bson.hasField(this->name().c_str())) {
        throw bson_error("Field '" + this->name() +
                         "' is not in the BSON object.");
      }
  ...
  template <typename U, typename ACCESSOR, typename CODER>
  static IndirectMember<U, ACCESSOR, CODER>* indirect_member(
      std::string const& name, ACCESSOR accessor, CODER coder) {
  ...
  template <typename U, typename GET_CONST, typename GET>
  static MemberFxnsDirect<U, GET_CONST, GET> member_fxns_direct(
      GET_CONST get_const, GET get) {
  ...
  template <typename U, typename GET_CONST, typename SET>
  static MemberFxnsIndirect<U, GET_CONST, SET> member_fxns_indirect(
      GET_CONST get_const, SET set) {
